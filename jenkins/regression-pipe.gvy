timestamps {
    node () {
        cleanWs()
    	stage ('Checkout') {
     	 checkout([$class: 'GitSCM', branches: [[name: '*/master']], doGenerateSubmoduleConfigurations: false, extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: '', url: 'https://github.com/YuriiShp/proxytcp.git']]]) 
    	}
    }
}
...
pipeline {
    ...
    stages {
        ...
        stage ('Run Tests'){
            steps{
                sh "sudo docker-compose -f ./ProxyTCP-automation-framework/docker-compose.yaml run --rm testenv pyats run job /pyats/project/src/jobs/regression.py --testbed-file /pyats/project/src/testbed.yaml"
            }
        }
    }
    post {

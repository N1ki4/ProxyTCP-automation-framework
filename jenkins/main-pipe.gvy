pipeline {
    ...
    stages {
        ...
        stage ('Run Tests'){
            steps{
                sh "sudo docker-compose -f ./ProxyTCP-automation-framework/docker-compose.yaml run --rm testenv pyats run job /pyats/project/src/jobs/main.py --testbed-file /pyats/project/src/testbed.yaml"
            }
        }
    }
    post {
